...
8. **Настроить регламентные задания в 1С:**
   - Выключить типовые регламентные задания, которые перенесены на HTTP-планировщик (обмен ценами, остатками, НСИ, Kafka, заказы)
   - Оставить только фискализацию (АТОЛ) и любые задания, требующие клиентского контекста
   - Предварительно проверить полный список заданий — какие из них требуют клиентского контекста (работа с оборудованием, UI)
9. Запустить службы
10. Проверить /health
